...
import {addDecisionName, deleteDecisionName, fetchDecisionNames, updDecisionName} from "./DecisionNameSlice";
...
const schema = yup.object().shape({
    dscName: yup.string().required('required')
});
...
const DecisionNameList = () => {
    ...
    const {decisionNames, loading, error} = useSelector(state => ({
        decisionNames: state.decisionName.decisionNames,
        loading: state.decisionName.loading,
        error: state.decisionName.error,
    }))
    ...
    const delDecisionName = () => {
        dispatch(deleteDecisionName(d.current.id)).then(unwrapResult)
        d.current = null
        setOpenDialog(() => false)
    }

    const createDecisionName = (data) => {
        dispatch(addDecisionName(data)).then(unwrapResult).then(setOpen(() => true))
        d.current = null

    }
    const updateDecisionName = (data) => {
        data.id = d.current.id
        dispatch(updDecisionName(data)).then(unwrapResult).then(setOpen(() => true))
        d.current = null
    }
    const submitForm = (data) => {
        return d.current
            ? updateDecisionName(data)
            : createDecisionName(data);
    }

    const fields = Object.keys(initialValues)
    const selectDecisionName = ({selectedRowsData}) => {
        ...
        <Aux>
            <Dialog open={openDialog} onClose={handleDialogClose} aria-labelledby="form-dialog-title">
                ...
                <DialogActions>
                    ...
                    <Buttonn onClick={(e) => delDecisionName(e)} color="primary">
                        {t('delete')}
                    </Buttonn>
                </DialogActions>
            </Dialog>
                ...
                <Col>
                    <Card>
                        <Card.Header style={{display: 'flex', justifyContent: 'space-between'}}>
                            <Card.Title as="h5">{t('decisionNames')}</Card.Title>
                                ...
                                <Col xs={12} md={4}>
                                    <div className={classes.paper}>
                                        ...
                                        <Typography component="h1" variant="h5">
                                            {d.current ? t('edit') : t('add')} {t('decisionName')}
                                        </Typography>
                                                    ...
                                                    <Grid container spacing={2}>
                                                        <Grid item xs={12}>
                                                            <TextField
                                                                ...
                                                                fullWidth
                                                                autoFocus
                                                                label={t('decisionName')}
                                                                name="dscName"
                                                                error={!!errors.dscName}
                                                                helperText={t(errors.dscName?.message)}
                                                            />
                                                        </Grid>
                                                    </Grid>
                                                </Form.Group>
                                ...
                                <Col xs={12} md={8} className={classes.rootForProg}>
                                    ...
                                    <Paper>
                                        <DataGrid id="dataGrid" ref={dg}
                                                  dataSource={decisionNames}
                                                  keyExpr="id"
                                                  focusedRowEnabled={true}
                                                  defaultSelectedRowKeys={null}
                                                  onSelectionChanged={selectDecisionName}
                                            ...
                                            <Column dataField="dscName" caption={t('decisionName')}/>
                                            <Column type="buttons">
...
export default DecisionNameList
